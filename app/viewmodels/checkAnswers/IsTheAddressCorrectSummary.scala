...
object IsTheAddressCorrectSummary {

  def row(answers: UserAnswers)(implicit messages: Messages): Option[SummaryListRow] =
    answers.get(IsTheAddressCorrectPage).map {
      answer =>
        val value = if (answer) "site.yes" else "site.no"

        SummaryListRowViewModel(
          key = "isTheAddressCorrect.checkYourAnswersLabel",
          ...
          actions = Seq(
            ActionItemViewModel("site.change",
              ...
              .withVisuallyHiddenText(messages("isTheAddressCorrect.change.hidden"))
          )
        )
    }
