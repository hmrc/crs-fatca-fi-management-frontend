...
import pages.addFinancialInstitution.{AddressLookupPage, NameOfFinancialInstitutionPage}
...
class SelectAddressControllerSpec extends SpecBase with MockitoSugar {
  ...
  "InstitutionSelectAddress Controller" - {
    ...
    "must redirect to manual UK address page if there are no address matches" in {
      ...
      redirectLocation(result).value mustEqual routes.UkAddressController.onPageLoad(NormalMode).url
    }
