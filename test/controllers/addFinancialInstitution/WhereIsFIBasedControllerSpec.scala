...
import views.html.addFinancialInstitution.WhereIsFIBasedView

import scala.concurrent.Future

class WhereIsFIBasedControllerSpec extends SpecBase with MockitoSugar {

  def onwardRoute = Call("GET", "/foo")

  lazy val whereIsFIBasedRoute = controllers.addFinancialInstitution.routes.WhereIsFIBasedController.onPageLoad(NormalMode).url

  val formProvider = new WhereIsFIBasedFormProvider()
  val form         = formProvider()
  private val ua   = emptyUserAnswers.set(NameOfFinancialInstitutionPage, "fiName").get

  "WhereIsFIBased Controller" - {

    "must return OK and the correct view for a GET" in {

      val userAnswers = ua.set(NameOfFinancialInstitutionPage, "fiName").success.value
      val application = applicationBuilder(userAnswers = Some(userAnswers)).build()

      running(application) {
        val request = FakeRequest(GET, whereIsFIBasedRoute)

        val result = route(application, request).value

        val view = application.injector.instanceOf[WhereIsFIBasedView]
